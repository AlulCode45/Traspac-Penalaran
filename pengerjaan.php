...
function wordCount($artikel, $kata) {
    $kataArray = explode(' ', strtolower($artikel));
    $kataJumlah = array_count_values($kataArray);
    return isset($kataJumlah[strtolower($kata)]) ? $kataJumlah[strtolower($kata)] : 0;
}

function replaceWord($artikel, $kataLama, $kataBaru) {
    return str_replace($kataLama, $kataBaru, $artikel);
}

function sortWord($artikel) {
    $kataArray = explode(' ', strtolower($artikel));
    sort($kataArray);
    return $kataArray;
}
...
$kataPencarian = "dan";
echo "Kata '$kataPencarian' ditemukan sebanyak: " . wordCount($artikel, $kataPencarian) . " kali.\n";

$kataLama = "adalah";
$kataBaru = "ialah";
echo "\nArtikel setelah penggantian:\n";
echo replaceWord($artikel, $kataLama, $kataBaru);

echo "\n\nKata-kata yang telah diurutkan:\n";
print_r(sortWord($artikel));
